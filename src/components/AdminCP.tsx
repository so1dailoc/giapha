          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-5">
              ...
              <div className="rounded-xl border border-slate-300 bg-white p-4 overflow-x-auto">
                <div className="text-[10px] uppercase tracking-wider font-black text-slate-500 mb-3">Xem trước thực tế — hai loại thẻ hoàn toàn độc lập</div>
                <div className="min-w-[560px] flex items-end justify-center gap-12 py-6">
                  <div className="text-center"><div className="text-[9px] font-bold text-slate-500 mb-2">THẺ NGANG</div><div style={{width:Math.min(Number((clanForm.defaultTreeSettings as any)?.horizontalCardWidth||260),340),height:Number((clanForm.defaultTreeSettings as any)?.horizontalCardHeight||110)||110,backgroundColor:(clanForm.defaultTreeSettings as any)?.horizontalCardBackgroundColor||'#5c0612',borderColor:(clanForm.defaultTreeSettings as any)?.horizontalCardBorderColor||'#d4a72c'}} className="rounded-xl border-2 p-3 shadow-lg flex flex-col justify-center"><div style={{fontSize:`${Math.max(9,Number((clanForm.defaultTreeSettings as any)?.horizontalCardFontSize||14))}px`,color:(clanForm.defaultTreeSettings as any)?.horizontalCardNameColor||'#fef3c7',backgroundColor:(clanForm.defaultTreeSettings as any)?.horizontalCardNameBackgroundColor||'transparent'}} className="font-black text-center px-1 rounded">NGUYỄN VĂN A</div><div className="text-[10px] text-center opacity-70 mt-1">Đời 5 • Trưởng nam</div></div></div>
                  <div className="text-center"><div className="text-[9px] font-bold text-slate-500 mb-2">THẺ DỌC</div><div style={{width:Math.min(Number((clanForm.defaultTreeSettings as any)?.verticalCardWidth||78),160),height:Number((clanForm.defaultTreeSettings as any)?.verticalCardHeight||180),backgroundColor:(clanForm.defaultTreeSettings as any)?.verticalCardBackgroundColor||'#5c0612',borderColor:(clanForm.defaultTreeSettings as any)?.verticalCardBorderColor||'#d4a72c'}} className="rounded-xl border-2 p-2 shadow-lg flex flex-col items-center justify-center"><div style={{fontSize:`${Math.max(9,Number((clanForm.defaultTreeSettings as any)?.verticalCardFontSize||13))}px`,color:(clanForm.defaultTreeSettings as any)?.verticalCardNameColor||'#fef3c7',backgroundColor:(clanForm.defaultTreeSettings as any)?.verticalCardNameBackgroundColor||'transparent'}} className="font-black text-center leading-tight">NGUYỄN<br/>VĂN<br/>A</div><div className="text-[8px] opacity-70 mt-2">Đời 6</div></div></div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-violet-200 bg-violet-50/40 p-4 space-y-4">
              ...
              <div className="rounded-xl border bg-white p-3 flex items-center gap-3"><div className="w-11 h-11 rounded-xl flex items-center justify-center shadow" style={{backgroundColor:clanForm.defaultTreeSettings?.interfacePrimaryColor || DEFAULT_INTERFACE_THEME.interfacePrimaryColor}}><Crown className="w-6 h-6" style={{color:clanForm.defaultTreeSettings?.interfaceAccentColor || DEFAULT_INTERFACE_THEME.interfaceAccentColor}}/></div><div><div className="font-black text-xs" style={{color:clanForm.defaultTreeSettings?.interfaceTextColor || DEFAULT_INTERFACE_THEME.interfaceTextColor}}>Preview giao diện mặc định</div><div className="text-[10px] text-slate-500">Các màu sẽ được áp dụng cho shell của website sau khi lưu.</div></div></div>
            </div>
