            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-5">
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  ...
                  <label className="text-[10px] font-semibold">Khoảng cách nút thu gọn dưới thẻ<input type="number" min={0} max={120} value={clanForm.defaultTreeSettings?.collapseControlOffset ?? 12} onChange={(e)=>setClanForm({...clanForm,defaultTreeSettings:{...clanForm.defaultTreeSettings,collapseControlOffset:Math.max(0,Math.min(120,Number(e.target.value)))}})} className="w-full mt-1 p-2 border rounded-lg bg-white" /><span className="block mt-1 text-[9px] text-slate-500">Khoảng cách từ đáy thẻ đến nhãn “−/＋ con”.</span></label>
                  <div className="sm:col-span-2 lg:col-span-3 mt-2 p-3 rounded-xl border border-indigo-200 bg-indigo-50/60"><div className="font-black text-xs text-indigo-950 mb-2">C. NỀN CÂY REACTFLOW — ĐỘC LẬP THEME WEBSITE</div><div className="grid grid-cols-1 sm:grid-cols-3 gap-3"><label className="text-[10px] font-semibold text-slate-700">Màu nền cây<input type="color" value={clanForm.defaultTreeSettings?.treeCanvasBackgroundColor || '#1e0205'} onChange={(e)=>setClanForm({...clanForm,defaultTreeSettings:{...clanForm.defaultTreeSettings,treeCanvasBackgroundColor:e.target.value}})} className="w-full h-9 mt-1 rounded-lg border p-1 bg-white"/></label><label className="text-[10px] font-semibold text-slate-700">Màu chấm / lưới<input type="color" value={clanForm.defaultTreeSettings?.treeCanvasGridColor || '#7b1113'} onChange={(e)=>setClanForm({...clanForm,defaultTreeSettings:{...clanForm.defaultTreeSettings,treeCanvasGridColor:e.target.value}})} className="w-full h-9 mt-1 rounded-lg border p-1 bg-white"/></label><label className="text-[10px] font-semibold text-slate-700">Khoảng chấm<input type="number" min={8} max={80} value={clanForm.defaultTreeSettings?.treeCanvasGridGap ?? 24} onChange={(e)=>setClanForm({...clanForm,defaultTreeSettings:{...clanForm.defaultTreeSettings,treeCanvasGridGap:Math.max(8,Math.min(80,Number(e.target.value)))}})} className="w-full p-2 mt-1 border rounded-lg bg-white"/></label></div><p className="text-[9px] text-indigo-800 mt-2">Nền cây và lưới này chỉ áp dụng cho vùng ReactFlow, không bị đổi theo preset giao diện website.</p></div>
              ...
              <div className="rounded-xl border border-slate-300 bg-white p-4 overflow-x-auto">
                ...
                <div className="min-w-[560px] flex items-end justify-center gap-16 py-6">
                  <div className="text-center">
                    ...
                    <div className="mt-2 inline-flex px-3 py-1 rounded-full border border-amber-500 bg-[#350207] text-amber-300 text-[9px] font-bold">⌃ − 4 con</div>
                  </div>
                  <div className="text-center">
                    ...
                    <div className="mt-2 inline-flex px-2 py-1 rounded-full border border-amber-500 bg-[#350207] text-amber-300 text-[8px] font-bold">⌃ − 4</div>
                  </div>
                </div>
              </div>
            </div>
